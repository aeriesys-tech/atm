...
const handleAddInputChange = (e) => {
  ...
  const handleUpdateSubmit = async (formData, setErrors, onSuccess) => {
    try {
      ...
      const apiErrors =
        err?.message?.errors || err?.response?.data?.errors || {};
      const newErrors = {
        role_group_id: apiErrors.role_group_id || "",
        role_code: apiErrors.role_code || "",
        role_name: apiErrors.role_name || "",
  ...
  const handleToggleStatus = async (row) => {
    try {
      ...
      const response = await axiosWrapper("api/v1/roles/deleteRole", {
        method: "POST",
        data: { id: row._id },
      });
      toast.success(response?.message || "Role status toggled successfully", {
        autoClose: 3000,
      });
      fetchRoles(currentPage, pageSize, sortBy, order, statusFilter);
    } catch (err) {
