...
const Master = () => {
  ...
  const fetchParameterTypes = async () => {
    try {
      const res = await axiosWrapper(
        "api/v1/parameter-types/getParameterTypes",
        {
          method: "POST",
        }
      );
    ...
    setFormValues((prev) => {
      if (name === "master_name") {
        ...
        return {
          ...
          model_name: modelNameManuallyEdited
            ? prev.model_name
            : derivedModelName,
        };
      }
  ...
  const handleCreateOrUpdateMaster = async (formData, setErrors, onSuccess) => {
    try {
      ...
      if (isEditMode) {
        ...
        response = await axiosWrapper("api/v1/masters/updateMaster", {
          method: "POST",
          data: payload,
        });
        toast.success(response?.message || "Master updated successfully", {
          autoClose: 3000,
        });
      } else {
        response = await axiosWrapper("api/v1/masters/createMaster", {
          method: "POST",
          data: payload,
        });
        toast.success(response?.message || "Master created successfully", {
          autoClose: 3000,
        });
      }

      // ✅ Re-fetch parameterTypes after create/update
      const paramTypes = await axiosWrapper(
        "api/v1/parameter-types/getParameterTypes",
        { method: "POST" }
      );
      ...
      const templateTypes = await axiosWrapper(
        "api/v1/template-types/getAllTemplateTypes",
        { method: "POST" }
      );
      ...
      toast.error(err?.message?.message || "Failed to create/update master", {
        autoClose: 3000,
      });
    } finally {
  ...
  const handleSoftDelete = async (row) => {
    try {
      ...
      const response = await axiosWrapper(`api/v1/masters/deleteMaster`, {
        method: "POST",
        data: { id: row._id },
      });
      ...
      await fetchMasters(currentPage, pageSize, sortBy, order, statusFilter);
      window.location.reload();
    } catch (error) {
      toast.error(error?.message || "Soft delete failed", { autoClose: 3000 });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleHardDelete = async (row) => {
    if (
      !window.confirm(
        "Are you sure you want to permanently delete this master?"
      )
    )
      return;
    ...
    try {
      ...
      // Delete from server
      const response = await axiosWrapper(`api/v1/masters/destroyMaster`, {
        method: "POST",
        data: { id: row._id },
      });
      ...
      toast.error(error?.message || "Hard delete failed", { autoClose: 3000 });
    } finally {
      setLoading(false);
    }
  };
                      ...
                      <table className="table table-bordered table-striped align-middle table-text">
                        <thead className="table-head align-middle">
                          <tr>
                            ...
                            <th>Unique</th>
                            <th>Tooltip</th>
                            {masterFieldData.length > 1 && <th>Action</th>}
                          </tr>
                        </thead>
                        <tbody>
